...
class Teensy_Ableton_Device_Controller(ControlSurface):
    __doc__ = " Script for Teensy_Ableton_Device_Controller in APC emulation mode "

    _active_instances = []
    def _combine_active_instances():
        track_offset = 0
        scene_offset = 0
        for instance in Teensy_Ableton_Device_Controller._active_instances:
    ...
    def _do_combine(self):
        if self not in Teensy_Ableton_Device_Controller._active_instances:
            Teensy_Ableton_Device_Controller._active_instances.append(self)
            Teensy_Ableton_Device_Controller._combine_active_instances()


    def _do_uncombine(self):
        if ((self in Teensy_Ableton_Device_Controller._active_instances) and Teensy_Ableton_Device_Controller._active_instances.remove(self)):
